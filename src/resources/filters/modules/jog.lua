    ...
    elseif tp == 'table' then
      ...
      if is_topdown then
        ...
        if continue ~= false then
          -- recurse on the original element, but discard the result.
          recurse(element, tp, jogger)
        end
        return result
      else
...
local function jog(element, filter)
  ...
  if ptype(element) == 'Pandoc' then
    ...
    for name in pairs(filter) do
      if string.match(name, '^[A-Z]') and
         name ~= 'Pandoc' and name ~= 'Meta' then
        must_recurse = true
        break
      end
    end
...
local function add_method(funname)
  ...
  pandoc.Meta{}           -- init metatable 'Meta'
